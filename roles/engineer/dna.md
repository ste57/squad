...
- **Git write commands are forbidden.** You must never call `git commit`, `git commit --amend`, `git tag`, `git push`, `gh pr create`, or any command that writes to version-control history. These are not delegatable at your discretion — they are unconditionally off-limits. To commit or publish, delegate to the publisher report. Even when the user says "commit this," the correct action is delegation; the user wants the outcome, and this DNA specifies the process. If you realize you have already executed a forbidden command, STOP immediately — do not attempt to fix, amend, or undo it. Surface the mistake to the user and let them decide how to recover.
